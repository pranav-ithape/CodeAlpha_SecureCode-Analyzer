...
const Dashboard: React.FC = () => {
  ...
  const totalFindings = scans.reduce((acc, scan) => acc + (scan.summary?.total || 0), 0);
  const criticalFindings = scans.reduce((acc, scan) => acc + (scan.summary?.critical || 0), 0);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-outline-variant pb-space-md">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            disabled={loading}
            className={`h-8 px-3 rounded-lg bg-surface-container border border-outline-variant hover:border-outline text-label-code-sm font-label-code-sm text-on-surface flex items-center gap-1.5 transition-colors ${loading ? 'opacity-70' : ''}`}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 rounded-xl bg-surface-container-low border border-outline-variant flex flex-col justify-between">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-bold font-headline-lg text-on-surface">{totalScans}</span>
        ...
        <div className="p-4 rounded-xl bg-surface-container-low border border-outline-variant flex flex-col justify-between">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-bold font-headline-lg text-on-surface">{totalFindings}</span>
            <span className="text-xs text-secondary font-label-code-sm font-semibold">{totalFindings === 0 ? 'Clean Baseline' : 'Needs attention'}</span>
          </div>
          <div className="mt-2 text-xs text-outline border-t border-outline-variant pt-2 flex items-center gap-1">
        ...
        <div className="p-4 rounded-xl bg-surface-container-low border border-outline-variant flex flex-col justify-between">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className={`text-3xl font-bold font-headline-lg ${criticalFindings > 0 ? 'text-error' : 'text-on-surface'}`}>{criticalFindings}</span>
